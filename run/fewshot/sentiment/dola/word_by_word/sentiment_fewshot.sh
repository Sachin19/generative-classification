...
for model in "${models[@]}"; do
    for setting in "${settings[@]}"; do
        ...
        TOKENIZERS_PARALLELISM=false python fewshot.py\
            ...
            --outputs_file "results/0923/fewshot/sentiment/$task/$dataset-$data_dir/$model/predictions.txt"\
            --results_file "results/0923/fewshot/sentiment/$task/$dataset-$data_dir/$model/results.jsonl"\
            --cc_outputs_file "results/0923/fewshot/sentiment/$task/$dataset-$data_dir/$model/cc_predictions.txt"\
            --model_dtype fp16\
            --pmi\
            --metric accuracy\
            --num_runs 10\
            ...
            #--bettertransformer\
        python plot_fewshot.py "results/0923/fewshot/sentiment/$task/$dataset-$data_dir/$model/results.jsonl" "results/0923/fewshot/sentiment/$task/$dataset-$data_dir/$model/results.png" "$task-$model"
        python csv_fewshot.py "results/0923/fewshot/sentiment/$task/$dataset-$data_dir/$model/results.jsonl" "results/0923/fewshot/sentiment/$task/$dataset-$data_dir/$model/results.txt" "$task-$model"
