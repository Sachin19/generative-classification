...
b2=(0.5)
for i in "${!b1[@]}"; do
    bash run/fewshot/nli/dola_tests/jsd_avg/nli_fewshot_jsd_avg_large.sh $task $dataset $data_dir $data_files $split $textfield1 $textfield2 $labelfield $label2id $jobid $label_names 1 ${b1[i]} ${b2[i]}
done
