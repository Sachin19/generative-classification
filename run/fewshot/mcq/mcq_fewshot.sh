...
for model in "${models[@]}"; do
    for setting in "${settings[@]}"; do
        ...
        python csv_fewshot.py "results/0923/fewshot/mcq/$task/$dataset-$data_dir/$model/results.jsonl" "results/0923/fewshot/mcq/$task/$dataset-$data_dir/$model/results.txt" "$task-$model"
    done
done
