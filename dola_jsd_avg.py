...
def get_nll_jsd_avg(model, tokenizer, batch, label_mask, num_labels, num_labelstrings, b1=0.25, b2=0.5, device="cuda"):
    # print("jsd_avg")
    with torch.no_grad():
        ...
        bucket_start = int(len(outputs.hidden_states) * b1)
        num_hidden_states = len(outputs.hidden_states)
        bucket_end = min(max(1, int((num_hidden_states-1)*b2)), num_hidden_states - 1)
        distances = torch.zeros(batch_size, bucket_end - bucket_start)

        for i, hidden_state in enumerate(outputs.hidden_states[bucket_start:bucket_end]):                
            ...
            for j in range(batch_size):
                ...
                # print(layer_logprobs.shape)
                # input()
                distances[j, i] = torch.mean(JSD(final_layer_logprobs, layer_logprobs))
                
                
        furthest_layer = torch.argmax(distances, dim=-1) # (BS)
        # print(distances)
        # print(furthest_layer)
        # print(outputs.hidden_states.shape)
        # input()
        # if torch.sum(furthest_layer != 0) > 0:
        #     print("NOT ZERO!")
        
        diffs = torch.stack([torch.nn.functional.log_softmax((final_layer[i] - to_log_probs(outputs.hidden_states[bucket_start + furthest_layer[i]][i])), dim=-1).contiguous() for i in range(batch_size)]).to(device)
