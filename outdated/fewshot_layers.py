...
from nli_fewshot_custom import get_tokenized_dataset_nli_fewshot_custom
...
def get_nll(model, tokenizer, batch, label_mask, num_labels, num_labelstrings):
    def nll_calc(shift_logprobs):
        # print(shift_logprobs.shape)
        # print(torch.sum(shift_logprobs[..., -2, :]))
        # input()
        ...
        # print(nll)
        # print(-shift_logprobs[0, range(shift_logprobs.shape[1]), shift_target.view(-1)])
        # input()
    ...
    def JSD(P, Q): 
        M = (P + Q)/2
        # print(P, P.shape)
        # print(torch.sum(torch.exp(P)))
        # print(Q, Q.shape)
        # print(torch.sum(torch.exp(Q)))
        # print(torch.nn.functional.kl_div(P, Q, log_target=True))
        # input()
        return (torch.nn.functional.kl_div(M, P, log_target=True) + torch.nn.functional.kl_div(M, Q, log_target=True))/2

    print(batch['input_ids'])
    print(tokenizer.decode(batch['input_ids'][0]))
    outputs = model(**batch, output_hidden_states=True)
    layers = [] # num_layers, num_labels
    distrs = []
    ...
    for i, hidden_state in enumerate(outputs.hidden_states):
        ...
        shift_logprobs = torch.nn.functional.log_softmax(logits, dim=-1).contiguous()
        distrs.append(shift_logprobs)
        layers.append(nll_calc(shift_logprobs).item())
    
    closest_layer = torch.argmax(torch.tensor([JSD(distrs[-1][..., -1, :], distrs[-i][..., -1, :]) for i in range(2, len(distrs)+1)]))+1
    print(closest_layer)
    input()
    
    diff = torch.nn.functional.log_softmax((distrs[-1] - distr[-closest_layer])[..., :-1, :], dim=-1).contiguous()

    # layers = torch.Tensor(layers)
    # print(layers.shape)
    # input()
    # logits = outputs.logits
    return nll_calc(diff) 
...
def main():
    ...
    if not custom:
        ...
        all_cc_layers = np.full((cf_shape[0], cf_shape[1], cf_shape[2]), None)
            ...
            for j in range(cf_shape[1]): # num_classes
                for k in range(cf_shape[2]): # num_sets
                    ...
                    for l in range(cf_shape[3]):
                        ...
                        tmp_thing = get_nll(model, tokenizer, cf_batch, cf_label_mask, 1, 1)
                        tmp_cfs.append(tmp_thing[0])
                        cfs_layers.append(tmp_thing[1])
                    # print(cfs_layers[0])
                    cfs_layers = torch.stack(cfs_layers)
                    # print(cfs_layers.shape)
                    # input()
                    cf_layers_avg = (torch.logsumexp(-torch.Tensor(cfs_layers), dim=0))
                    cf_layers_avg = np.log(num_cf) - cf_layers_avg 

                    # num_layers = len(cfs_layers[0])
                    # cf_layers_avg = []
                    # for layer in range(num_layers):
                    #     tmp_layer = cfs_layers[layer]
                    #     cf_layer_avg = (torch.logsumexp(-torch.Tensor(tmp_layer), dim=0))
                    #     cf_layer_avg = np.log(num_cf) - cf_layer_avg 
                    #     cf_layers_avg.append(cf_layer_avg)
                    # cf_layers_avg = torch.Tensor(cf_layers_avg)
                    cf_layers_avg.to(device)
                    # print(cf_layers_avg.shape)

                    all_cc_layers[i, j, k] = cf_layers_avg
                    # print(all_cc_layers[i, j, k].shape)
                    # input()
        ...
        for num_examples in range(args.k+1):
            ...
            for batch in tqdm(eval_dataloader):
                ...
                if args.batch_by_labelstring:
                    ...
                    if not custom:
                        cc_nll = np.full((batch['input_ids'].shape[0], num_labels, total_perm_sets), None)
                    layers = np.full((num_labels, total_perm_sets), None)
                    for i in range(num_labels): # for each label
                        for j in range(total_perm_sets): # for each prompt per label
                            ...
                            val, layers[i, j] = get_nll(model, tokenizer, sub_batch, label_mask_ij, 1, 1)
                            # plt.plot(range(0, len(layers)), layers)
                            # title = tokenizer.decode(batch["input_ids"].squeeze())
                            # plt.title(tokenizer.decode(batch["input_ids"].squeeze()))
                            # print(title)
                            # plt.savefig(f"test/{title[:10]}-{title[-10:]}.png")
                            # plt.clf()
                            nll[:, i, j] = val.squeeze()
                            # if not custom:
                            #     cc_nll[0, i, j] = nll[:, i, j] - all_cc_layers[num_examples, i, j][-1]
                            #     print(all_cc_layers[num_examples, i, j][-1], cc_nll[0, i, j], nll[:, i, j])
                            #     input()
                                # all_cc[num_examples, i, j]
                    # if example_num == 0:
                    min_cc = float("inf")
                    min_lab = 0
                    for l in range(num_labels):
                        plt.plot(range(0, len(layers[l, 0])), layers[l, 0], label=args.label_names.split(",")[l])
                        if not custom: 
                            plt.plot(range(0, len(layers[l, 0])), layers[l, 0] - all_cc_layers[num_examples, l, 0], label="cc-"+args.label_names.split(",")[l])
                            if (layers[l, 0] - all_cc_layers[num_examples, l, 0])[-1] < min_cc:
                                min_cc = (layers[l, 0] - all_cc_layers[num_examples, l, 0])[-1]
                                min_lab = l
                    # plt.plot(range(0, len(layers[1, 0])), layers[1, 0], label=args.label_names.split(",")[1])
                    if custom:
                        ax = plt.gca()
                        ax.set_ylim([0, 60])
                    plt.legend(loc="best")
                    tmp_prd = batch['input_ids'][0, num_examples, 0, 0, :] * batch['attention_mask'][0, num_examples, 0, 0, :]
                    title = tokenizer.decode(tmp_prd[tmp_prd != 0])
                    prediction = args.label_names.split(',')[nll[0, :, 0].min(dim=0)[1]]
                    cc_prediction = args.label_names.split(',')[min_lab]
                    GT = args.label_names.split(',')[int(labels[0])]
                    plt_title = f"{title[:50]}...\n GT: {GT}, Pred: {prediction}"
                    if not custom: 
                        plt_title += f", cc_pred: {cc_prediction}"
                    plt.title(plt_title)
                    alt = ""
                    custom_string = "dir-"
                    if custom:
                        custom_string = "custom-"
                    if "entailment" in args.label_names.split(","):
                        alt = "alt_lab-"

                    fig_save_dir = f"test/{args.dataset}/{args.data_dir}/{alt}{args.task}/{args.model}/ICL{num_examples}/{prediction==GT}/set{0}"
                    if not os.path.exists(fig_save_dir):
                        os.makedirs(fig_save_dir)
                    plt.savefig(fig_save_dir + f"/example{example_num}.png")
                    plt.clf()
                ...
                #     # if setid == 0 and example_num==0:
                #     #     print(f"prediction: {args.label_names.split(',')[int(results[0])]}")
                #     #     input()
                #     if not custom:
