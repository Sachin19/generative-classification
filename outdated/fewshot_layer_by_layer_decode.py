...
def get_nll(model, tokenizer, batch, label_mask, num_labels, num_labelstrings, _lambda=2): # lower means more likely, so here get_nll would be the slope of the last '_lambda' layers
    def nll_calc(logits):
    ...
    neg_slopes = torch.Tensor([((layers[-1] - layers[-i])/(i-1)) for i in range(2, _lambda+1)])
