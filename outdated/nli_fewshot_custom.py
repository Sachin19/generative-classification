        ...
        for i in range(_num_sets):
            ...
            for j in range(_k):
                ...
                for l in range(num_classes):
                    few_shot_strs[j] += template[l].format(premise="\""+few_shot["premise"][l][j]+"\"", hypothesis="\""+few_shot["hypothesis"][l][j]+"\"") + "\n\n"
            # print(few_shot_strs)
            # input()
            ...
            for num_examples in range(_k+1):
                for j in range(num_classes):
                    ...
                    for k in range(_num_sets):
                        string = few_shot_examples[num_examples, k] + template[j].format(premise=premise, hypothesis=hyp)
                        # print(string)
