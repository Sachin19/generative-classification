...
class ScriptArguments:
    ...
    _lambda: Optional[int] = field(default=10, metadata={"help": "num layers to use for the area calc"})
...
def get_nll(model, tokenizer, batch, label_mask, num_labels, num_labelstrings, _lambda=10): # lower means more likely, so here get_nll would be the approx area under the slope of the last '_lambda' layers
    def nll_calc(logits):
    ...
    layers = torch.Tensor([nll_calc(model.lm_head(hidden_state)).item() for hidden_state in outputs.hidden_states]) # num_layers, num_labels
    # print(len(outputs.hidden_states))
    # input()
    # for i, hidden_state in enumerate(outputs.hidden_states):
    #     # print(i, hidden_state.shape)
    #     logits = 
    #     layers.append(nll_calc(logits).item())
    
    
    layers = torch.Tensor(layers)
    areas = torch.tensor([sum(layers[-(i+1):]) for i in range(_lambda)])
    area = layers[-_lambda:].sum() #cumsum
    # print(layers.shape)
    # input()
...
def main():
    ...
    if not custom:
        ...
        for j in range(cf_shape[1]): # num_classes
            for k in range(cf_shape[2]): # num_sets
                tmp_cfs = torch.zeros(cf_shape[3], args._lambda - 1)
    ...
    accurate = { 
        'results': [[0 for _ in range(args._lambda-1)] for setid in range(args.num_sets)],
        }

    all_predictions = {
        'results': [[[] for _ in range(args._lambda-1)] for setid in range(args.num_sets)],
        }
    ...
    with torch.no_grad():            
        ...
        for batch in tqdm(eval_dataloader):
            ...
            if args.batch_by_labelstring:
                nll = torch.empty(batch['input_ids'].shape[0], num_labels, total_perm_sets, args._lambda-1).to(device)
                if not custom:
                    cc_nll = torch.empty(batch['input_ids'].shape[0], num_labels, total_perm_sets, args._lambda-1).to(device)
    ...
    for setid in range(args.num_sets):
        ...
        for layer_num in range(args._lambda-1):
