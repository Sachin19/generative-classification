...
def process_batch(batch, alllabelstrings_tokenized, i, j, device, tokenizer=None):
    labelstring_tokenized = alllabelstrings_tokenized[i][j] # (2, 11) as expected
    
    if args.debug:
        print(labelstring_tokenized)
        print(tokenizer.batch_decode(labelstring_tokenized.input_ids))
        input("labelstring")
    batch = {k: v.to(device) for k, v in batch.items()} # just using cuda
    ...
    # print(expanded_label_input_ids)
        ...
        for batch in tqdm(eval_dataloader):
            ...
            for runid in range(args.num_runs):  # we only compute probs once and compute mean and var by grouping them for the x-axis values              
                for k in range(init_index, end_index): # 0, 10 (we don't use 11 as there would be no variance, not ideal though due to the differences in 11 C k) 
                    ...
                    loss = -torch.logsumexp(-nll_subset, dim=1) + np.log(k) # summing over nl probabilities. To sum, need to convert nll to ll, then exponentiate, them sum, then log again. To prevent underflow (batch size; no. labels).T
                    result_logsumexp = loss.min(dim=0)[1] # an array of label indices, i.e. the prediction
                    if args.debug:
                        print(loss, result_logsumexp)

                    #average or geometric mean of probabilities
                    loss = torch.mean(nll_subset, dim=1) # just different processing
