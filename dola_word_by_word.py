...
def get_nll_word_by_word(model, tokenizer, batch, label_mask, num_labels, num_labelstrings, b1=0.25, b2=0.5, device="cuda"):
    with torch.no_grad():
        ...
        def JSD(P, Q): 
            M = torch.logsumexp(torch.stack((P, Q)), dim=0) - np.log(2)
            val1 = torch.nn.functional.kl_div(M, P, log_target=True, reduction="none")/2
            val2 = torch.nn.functional.kl_div(M, Q, log_target=True, reduction="none")/2
            val = torch.sum(val1 + val2, dim=-1)
            return val
        
        def to_log_probs(hidden_state):
        ...
        final_layer = to_log_probs(outputs.hidden_states[-1])

        zone_start = int(len(outputs.hidden_states) * b1)
        zone_end = int((len(outputs.hidden_states)*b2)-1)

        batch_size = batch['input_ids'].shape[0]
        num_words = batch['input_ids'].shape[1]-1
        distances = torch.zeros(batch_size, num_words, zone_end - zone_start).to(torch.float16).to(device)
        

        for i, hidden_state in enumerate(outputs.hidden_states[zone_start:zone_end]):
            shift_logprobs = to_log_probs(hidden_state)
            num_words = shift_logprobs.shape[-2]
            
            for j in range(batch_size):
                final_layer_logprobs = final_layer[j, ..., label_mask[j, ..., 1:] == 1, :]
                layer_logprobs = shift_logprobs[j, ..., label_mask[j, ..., 1:] == 1, :] # (num_1_words, vocab)
                # val = JSD(final_layer_logprobs, layer_logprobs)
                # print(val.dtype, distances.dtype)
                # input()

                distances[j, label_mask[j, ..., 1:]==1, i] = JSD(final_layer_logprobs, layer_logprobs)

        
        # furthest_layer = torch.argmax(distances, dim=-1) # (BS, num_words)
        # max_dist_layer = torch.max(distances, dim=-1)
        max_dist_layer, furthest_layer = distances.max(dim=-1)

        final_logits = final_layer.clone()
        for i in range(batch_size):
            for j in torch.nonzero(max_dist_layer[i]).flatten():

                tmp_val = torch.nn.functional.log_softmax(model.lm_head(outputs.hidden_states[furthest_layer[i, j]][i, j]), dim=-1).contiguous()
                final_logits[i, j] = torch.nn.functional.log_softmax(final_logits[i, j] - tmp_val, dim=-1).contiguous()

        # if torch.sum(furthest_layer != 0) > 0:
        #     print("NOT ZERO!")
        # diffs = torch.stack([torch.nn.functional.log_softmax((final_layer[i] - distrs[furthest_layer[i]][i]), dim=-1).contiguous() for i in range(batch_size)]).to(device)
        diffs = final_logits
        return nll_calc(diffs)
