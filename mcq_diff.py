# mcq but where all the choices are in one column
import os
...
class ScriptArguments:
    ...
    question: Optional[str] = field(default="sentence1", metadata={"help": "ok"})
    choices: Optional[str] = field(default="sentence2", metadata={"help": "ok"})
...
def get_tokenized_dataset(raw_dataset, tokenizer, question="question", choices="choices", labelfield="label", label2id=None, space=False, choice_array="text", label_array="label"):
    def preprocess_function(examples): # would need to keep the prem and hyp separate, or just get the mask here
        ...
        np.random.seed(2024)   
        examples_choices = examples[choices][:test_n]
        examples_questions = examples[question][:test_n]
        
        pad_token_id = tokenizer.pad_token_id
        all_labelstrings = TASK2LABELSTRINGS[args.task][0] # (num classes by num_label_strings)
        ...
        for i in range(n):
            l_for_length_calc = len(examples[choices][i]["text"])
            if l_for_length_calc > num_classes:
                num_classes = l_for_length_calc

        choices_maxed = np.full((n, num_classes), None)
        for i in range(n):
        ...
        for i in range(n):
            for j in range(num_classes):
                quoted_question = "\"" + examples[question][i] + "\""
                quoted_choice = "\"" + choices_maxed[i][j] + "\""
                if args.aGivenQ:
                ...
                for k in range(num_labelstrings):
                    tmp_tok = tokenizer(all_labelstrings[k].format(text1=quoted_masked, text2=quoted_target))
                    ...
                    idx = len(tokenizer.encode("x " + quoted_target, add_special_tokens=False)[len(tok_x):])
                    tmp_label_mask[-(idx):] = 1
                    label_masks[i, j, k] = tmp_label_mask
                    
            label_names = examples[choices][i]["label"]
            labels[i] = label_names.index(examples[labelfield][i])
        max_len = 0
        for i in range(n):
            for j in range(num_classes):
                for k in range(num_labelstrings):
        ...
        # quoted_question = "\"" + examples[question][test_i] + "\""
        # quoted_choice = "\"" + choices_maxed[test_i][test_j] + "\""
        # if args.aGivenQ:
        #     quoted_masked, quoted_target = quoted_question, quoted_choice
        # else:
        #     quoted_masked, quoted_target = quoted_choice, quoted_question
        # print(tokenizer(s_tmp.format(text1=quoted_masked, text2=quoted_target)))
        ...
        # print(splitted[0][3])
        # print(quoted_target)
        # print(tokenizer(quoted_target, add_special_tokens=False)["input_ids"])
        # print(quoted_masked)
...
def main():
    ...
    tokenized_dataset = get_tokenized_dataset(raw_dataset, tokenizer, args.question, args.choices, args.labelfield, label2id, ("gpt" in args.model or "pythia" in args.model or "opt" in args.model) and ("hate" in args.task))
            ...
            if args.batch_by_labelstring:
                ...
                for i in range(num_labels): # for each label
                    ...
                    for j in range(num_labelstrings): # for each prompt per label
                        ...
                        sub_batch={}
                        sub_batch['input_ids'], sub_batch['attention_mask'], label_mask_ij = batch['input_ids'][idx], batch['attention_mask'][idx], label_mask[idx]
