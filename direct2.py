...
class DataCollatorForDirectClassification:  
    ...
    def __call__(self, features):
        ...
        # print("!!!!!!!!!!!!!!!!!!!!")
        # print(len(new_features['input_ids'][0]))
        # print(len(new_features['input_ids'][1]))
        # print(batch['input_ids'].shape)
        return batch

def get_tokenized_dataset(raw_dataset, tokenizer, textfield1="sentence1", textfield2="sentence2", labelfield="label", label2id=None):
    def preprocess_function(examples):
        print(len(examples[args.textfield1]))
        x = tokenizer([TASK2PROMPT[args.task.split("#")[0]][0].format(text1=example1, text2=example2) for (example1, example2) in zip(examples[textfield1], examples[textfield2])], truncation=True)
...
def main():
    ...
    tokenized_dataset = get_tokenized_dataset(raw_dataset, tokenizer, args.textfield1, args.textfield2, args.labelfield, label2id)
    print("datasets and tokenizer loaded")
    
    # i = 0
    # for thing in tokenized_dataset:
    #     if i == 10:
    #         break
    #     print(thing['label'])
    #     print(thing['input_ids'].shape)
    #     print(thing['attention_mask'].shape)
    #     print()
    #     i += 1
    ...
    if args.model_dtype == "bit4":
        ...
        model = AutoModelForCausalLM.from_pretrained(args.model, load_in_8bit=True, cache_dir="models/", token=TOKEN, device_map="auto")
        device = model.device
    else:
    ...
    for runid in range(args.num_runs):
        result = { 
            ...
            "k": [],
            'direct_f1_arithmetic': [],
            'direct_f1_geometric': [],
            'direct_f1_harmonic': [], 
            'direct++_f1_arithmetic': [],
            'direct++_f1_geometric': [],
            'direct++_f1_harmonic': [], 
            'direct_accuracy_arithmetic': [],
            'direct_accuracy_geometric': [],
            'direct_accuracy_harmonic': [], 
            'direct++_accuracy_arithmetic': [],
            'direct++_accuracy_geometric': [],
            'direct++_accuracy_harmonic': [], 
        ...
        for k in range(1, maxk+1):
            ...
            for m in ['f1', 'accuracy']:
                result['direct_'+m+'_arithmetic'].append(compute_metric(cm_direct_arithmetic, m))
                result['direct_'+m+'_geometric'].append(compute_metric(cm_direct_geometric, m))
                result['direct_'+m+'_harmonic'].append(compute_metric(cm_direct_harmonic, m))

                result['direct++_'+m+'_arithmetic'].append(compute_metric(cm_directpp_arithmetic, m))
                result['direct++_'+m+'_geometric'].append(compute_metric(cm_directpp_geometric, m))
                result['direct++_'+m+'_harmonic'].append(compute_metric(cm_directpp_harmonic, m))
